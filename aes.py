...
def aes (m, k):
    my_subyte = SubBytes(m,s_box)
    return bitwise_xor(my_subyte, k)

def inv_aes(m,k):
    inv_add_key = bitwise_xor(m, k)
    my_message = SubBytes(inv_add_key, inv_s_box)
    return my_message
...
def d(cipher_path, key_path, output_path):
    ...
    message = read_as_byte(cipher_path)
    plaintext = inv_aes(inv_aes(message,k2),k1)
    write_as_byte(output_path, plaintext)


######## CHECKINGS #######
e("message_long.txt", "keys_long.txt", "test_cipher_long.txt")